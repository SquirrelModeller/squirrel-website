<script lang="ts">
  ...
  import { loadState, saveState, currentDay, keyTimeRemaining, pickStory } from '$lib/saga/state'
  import { findNextScene, applyEffect, completeScene, resolveText } from '$lib/saga/engine'
  ...
  let suppressed: boolean = $derived(
    ...
    (isBlogPost(page.url.pathname) && !sagaState.completedScenes.includes('day0_cookies'))
  )
  ...
  function tryOpen() {
    ...
    if (s.id === 'day2_key' && !sagaState.keyFound) {
      if (!sagaState.keyPage) {
        sagaState = { ...sagaState, keyPage: pickKeyPage(), keyStartedAt: new Date().toISOString() }
        saveState(sagaState)
      }
      startKeyTimer()
    }
  ...
  function getChoices(s: Scene) {
    ...
    if (s.id === 'day2_key') {
      ...
      return s.choices.filter(c => c.condition?.(sagaState) ?? true).filter(c => {
        if (c.condition) return c.condition(sagaState)
        return true
      }).filter(c => {
        // hide "found key" choices if key not found, hide "timed out" choices if key found
        if ((c.label === 'here. try not to lose it again.' || c.label === 'you owe me.') && !sagaState.keyFound) return false
        if ((c.label === 'pay the locksmith.' || c.label === 'he can sleep in the hallway. one night.') && (sagaState.keyFound || !timedOut)) return false
        return true
      })
    }
  ...
  function afterReaction() {
    ...
    if (!hasNext) {
      // no more scenes today — advance narrative day so next session shows day+1
      sagaState = { ...sagaState, narrativeDay: sagaState.narrativeDay + 1 }
      saveState(sagaState)
    }
  ...
  let yelpReview = $derived(
    scene !== null && scene.id === 'day10_yelp' ? getYelpReview(sagaState.spite) : null
  )

  let isBrawl = $derived(scene !== null && scene.brawl === true && !showingReaction)
  let isKeyScene = $derived(scene !== null && scene.id === 'day2_key' && !showingReaction)
...
{#if open && scene && !suppressed}
  ...
  <div class="saga-popup" class:brawl={isBrawl} role="dialog" aria-modal="true" aria-label="Cookie saga">
    ...
    {:else}
      <div class="saga-text">
        ...
        {#if yelpReview}
          <div class="yelp-stars">{'⭐'.repeat(yelpReview.stars)}{'☆'.repeat(5 - yelpReview.stars)}</div>
          <p class="yelp-text">"{yelpReview.text}"</p>
        {/if}

        {#if isKeyScene && !sagaState.keyFound}
          <div class="key-timer" class:urgent={keyTimer < 60} class:expired={keyTimedOut}>
            {#if keyTimedOut}
              <span>time's up.</span>
            {:else}
      ...
      <div class="saga-choices" class:jitter={isBrawl}>
        {#each getChoices(scene) as choice}
          ...
          >
            {typeof choice.label === 'function' ? choice.label(sagaState) : choice.label}
          </button>
        {/each}
      </div>
    {/if}
...
<style>
  ...
  .saga-popup.brawl {
    animation: saga-enter 0.25s ease, saga-shake 0.5s ease 0.3s;
  }
  ...
  .saga-text br { display: block; margin: 0.4rem 0; content: ''; }
  ...
  .saga-choices.jitter .saga-choice:nth-child(2) { animation-delay: 0.65s; }
  .saga-choices.jitter .saga-choice:nth-child(3) { animation-delay: 0.7s; }
  ...
  .key-timer.urgent { color: #e05a5a; border-color: #e05a5a; }
  .key-timer.expired { color: var(--text-muted); border-color: var(--border); opacity: 0.6; }
